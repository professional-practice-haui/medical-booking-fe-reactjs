...
const handleConvertWorkingTime = (workingTime) => ({
  id: workingTime.id || null,
  workingPlan: workingTime.workingPlan?.id || null,
  startTime: workingTime.startTime || '',
  endTime: workingTime.endTime || '',
  maxSlots: workingTime.maxSlots || '',
});

const UpdateWorkingTimeModal = ({
  selectedRow,
  isOpenUpdateModal,
  ...
  const [workingTimeInfo, setWorkingTimeInfo] = useState(
    handleConvertWorkingTime(selectedRow),
  );

  useEffect(() => {
    setWorkingTimeInfo(handleConvertWorkingTime(selectedRow));
  }, [selectedRow]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const { id, ...remainingWorkingTimeInfo } = workingTimeInfo;

    try {
      const response = await fetch(
        import.meta.env.VITE_API_URL + `/working-times/${id}`,
        {
          method: 'PUT',
          headers: {
          ...
          body: JSON.stringify(pick(remainingWorkingTimeInfo)),
        },
      );
                ...
                <div className="w-full">
                  ...
                  <div className="relative z-20 bg-transparent dark:bg-form-input">
                    <select
                      ...
                      name="workingPlan"
                      value={workingTimeInfo.workingPlan || ''}
                ...
                <div className="w-full">
                  ...
                  <div className="relative z-20 bg-transparent dark:bg-form-input">
                    <select
                      className="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary"
                      name="startTime"
                      value={workingTimeInfo.startTime || ''}
                      onChange={handleChange}
                      required
                    >
                      <option value="">-- Chọn --</option>
              ...
              <div className="mb-5.5 flex flex-col gap-5.5 sm:flex-row">
                <div className="w-full">
                  ...
                  <input
                    ...
                    name="maxSlots"
                    value={workingTimeInfo.maxSlots}
                    onChange={handleChange}
                    required
                  />
                </div>
