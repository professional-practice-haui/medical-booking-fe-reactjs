...
const handleConvertDoctor = (doctor) => ({
  ...
  department: doctor.department?.id || null,
});
...
const UpdateDoctorModal = ({
  ...
  const [doctorInfo, setDoctorInfo] = useState(
    handleConvertDoctor(selectedRow),
  );

  useEffect(() => {
    setDoctorInfo(handleConvertDoctor(selectedRow));
  }, [selectedRow]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const { id, ...remainingDoctorInfo } = doctorInfo;

    const data = new FormData();
    for (let key in remainingDoctorInfo) {
      data.append(key, remainingDoctorInfo[key]);
    }
          ...
          <form onSubmit={handleSubmit}>
            <div className="relative p-4">
              ...
              <div className="mb-5.5 flex flex-col gap-5.5 sm:flex-row">
                <div className="w-full">
                  <label className="mb-2.5 block text-base font-bold text-black dark:text-white">
                    Bằng cấp <span className="text-meta-1">*</span>
                  </label>
                  <input
                    type="text"
                    className="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary"
              ...
              <div className="mb-5.5 flex flex-col gap-5.5 sm:flex-row">
                <div className="w-full">
                  <label className="mb-2.5 block text-base font-bold text-black dark:text-white">
                    Năm kinh nghiệm <span className="text-meta-1">*</span>
                  </label>
                  <input
                    type="text"
                    className="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary"
