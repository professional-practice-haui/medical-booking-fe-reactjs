...
const handleConvertWorkingPlan = (workingPlan) => ({
  id: workingPlan.id || null,
  doctor: workingPlan.doctor?.id || null,
  date: workingPlan.date || '',
  place: workingPlan.place || '',
});

const UpdateWorkingPlanModal = ({
  selectedRow,
  isOpenUpdateModal,
  ...
  const [workingPlanInfo, setWorkingPlanInfo] = useState(
    handleConvertWorkingPlan(selectedRow),
  );

  useEffect(() => {
    setWorkingPlanInfo(handleConvertWorkingPlan(selectedRow));
  }, [selectedRow]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const { id, ...remainingWorkingPlanInfo } = workingPlanInfo;

    try {
      const response = await fetch(
        import.meta.env.VITE_API_URL + `/working-plans/${id}`,
        {
          method: 'PUT',
          headers: {
          ...
          body: JSON.stringify(pick(remainingWorkingPlanInfo)),
        },
      );
              ...
              <div className="mb-5.5 flex flex-col gap-5.5 sm:flex-row">
                <div className="w-full">
                  <label className="mb-2.5 block text-base font-bold text-black dark:text-white">
                    Tên bác sĩ <span className="text-meta-1">*</span>
                  </label>
                  <div className="relative z-20 bg-transparent dark:bg-form-input">
                    <select
                      className="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary"
                      name="doctor"
                      value={workingPlanInfo.doctor || ''}
              ...
              <div className="mb-5.5 flex flex-col gap-5.5 sm:flex-row">
                <div className="w-full">
                  ...
                  <input
                    ...
                    name="date"
                    value={
                      workingPlanInfo.date &&
                      format(new Date(workingPlanInfo.date), 'yyyy-MM-dd')
                    }
                    onChange={handleChange}
                    required
                  />
...
export default UpdateWorkingPlanModal;
